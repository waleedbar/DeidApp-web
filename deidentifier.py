...
class AdvancedHybridDeidentifierV10_3:
    ...
    def _build_strict_rejection_list(self) -> Set[str]:
        return {
            'emergency department', 'liver enzymes', 'enzymes', 'enzym', 'brucella titer', 'checkenpox', 'l.f.t', 
            'positive', 'negative', 'medlabs', 'contents', 'المريضه', 'available', 'avai lable', 'unknown', 
            'pt', 'doctor', 'doctors', 'consult', 'see order', 'medical care', 'muscle strain', 's.o.a.p', 
            'soap', 'تقريرًا', 'abdominal discomfort', 'abdominal pain', 'gastritis', 'functional pain',
            'epigastric pain', 'chest pain', 'back pain', 'joint pain', 'headache', 'nausea', 'vomiting', 
            'diarrhea', 'constipation', 'dyspepsia', 'internal medicine', 'family medicine', 'pediatrics', 
            'surgery', 'orthopedics', 'cardiology', 'neurology', 'gastroenterology', 'ultrasound abdomen', 
            'ultrasound', 'ct scan', 'mri', 'x-ray', 'blood test', 'urine test', 'ecg', 'echo', 'endoscopy',
            'hepatosplenomegaly', 'cardiomegaly', 'lymphadenopathy', 'organomegaly', 'edema', 'inflammation', 
            'infection', 'university', 'hospital', 'clinic', 'center', 'building', 'street', 'st', 'city', 
            'town', 'village', 'district', 'area', 'zone', 'no known', 'not known', 'follow up', 'follow-up', 
            'advised', 'discharge', 'admission', 'consultation', 'referral', 'general eligibility', 
            'verified', 'rt tex', 'report text', 'غير متوفر', 'غير-متوفر', 'not available', 'n/a'
        }

    def _build_medical_terms_list(self) -> Set[str]:
        return {
            'abdominal', 'gastric', 'cardiac', 'hepatic', 'renal', 'pulmonary', 'neurological', 
            'dermatological', 'orthopedic', 'ophthalmic', 'pain', 'discomfort', 'syndrome', 'disease', 
            'disorder', 'condition', 'ultrasound', 'radiography', 'tomography', 'endoscopy', 'biopsy',
            'inflammation', 'infection', 'edema', 'hemorrhage', 'ischemia', 'diagnosis', 'prognosis', 
            'treatment', 'therapy', 'medication'
        }

    def _build_known_locations(self) -> Set[str]:
        return {
            'عمان', 'الزرقاء', 'إربد', 'العقبة', 'المفرق', 'الكرك', 'معان', 'جرش', 'amman', 'zarqa', 
            'irbid', 'aqaba', 'mafraq', 'karak', 'maan', 'jerash', 'تلاع العلي', 'الجبيهة', 'الصويفية', 
            'الشميساني', 'عبدون', 'مرج الحمام', 'الرابية', 'خلدا', 'الرصيفة', 'حكما', 'الطويل', 'المشاقبه', 'مضافه'
        }
    ...
    def load_models(self):
        ...
        # This part related to Google Colab is commented out as it's not needed on a server
        # if IN_COLAB and not os.path.exists("/content/drive/MyDrive"):
        #     drive.mount('/content/drive')
            
        try:
            # THIS IS THE ONLY MAJOR CHANGE: Point to the Hugging Face Hub model ID
            my_trained_model_path = "WaleedBR34/my-ultimate-ner-model"
            
            # The code will now attempt to load this from the Hub
            ...
            # The rest of the models are loaded as usual
    ...
    def extract_entities_regex(self, text: str) -> List[Dict]:
        ...
        patterns = {
            ...
            'ID': [r'#\s*(\d{7,12})\b', r'\b(9\d{8,9})\b', r'(?:رقم وصل كشفيه|National ID|MRN|SSN)\s*:?\s*\(?#?\s*(\d{6,12})\s*\)?', r'\b\d{3}-\d{2}-\d{4}\b'],
            'PHONE': [r'(?<![0-9])(?:\+?962|0)\s*7[789][\s-]?\d[\s-]?\d[\s-]?\d[\s-]?\d[\s-]?\d[\s-]?\d[\s-]?\d(?![0-9])', r'(?<![0-9])\+962\s?[6-7](?:[\s-]?\d){7,8}(?![0-9])'],
            'DATE': [r'\b\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\b', r'\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2},?\s+\d{4}(?:@\d{2}:\d{2})?'],
            'AGE': [r'\b\d{1,2}yr\b', r'\b\d{1,2}yr\s+\d{1,2}mo\b', r'\b\d{1,2}\s+MONTHS?\s+OLD\b', r'\b\d{1,2}\s+YEARS?\s+OLD\b', r'\(\s*\d+\s*\)\s*yrs\b', r'\b\d{1,2}mo\b', r'\b\d{1,2}wk\b', r'\b\d{1,3}yrs\b'],
            'HCW': [r'\bالدكتور\s+([\u0600-\u06FF]+(?:\s+[\u0600-\u06FF]+){0,2})\b', r'\b(?:Dr|Doctor)\s*\.?\s*([A-Z][a-zA-Z\-]+(?:\s+[A-Z]\.?)?(?:\s+[A-Z][a-zA-Z\-]+){0,2})\b'],
            'PERSON': [r'\b[\u0600-\u06FF]+,(?:[\u0600-\u06FF]+(?:[\s-]|-?الدين)?)+\b', r'\b(?:ابو|أم)-[\u0600-\u06FF]+(?:,\s?[\u0600-\u06FF]+)+\b'],
            'ADDRESS': [
                r'(?:Address|العنوان)\s*:?\s*([^\n]{10,100})', r'[\u0600-\u06FF\s]+-\s*[\u0600-\u06FF\s]+-\s*شارع\s+[\u0600-\u06FF\s]+-\s*عمارة\s+\d+',
                r'[\u0600-\u06FF]+/[\u0600-\u06FF]+-[\u0600-\u06FF\s,]+', r'\b(?:شارع|حي|منطقة|عمارة|بناية)\s+[\u0600-\u06FF\s]+(?:،|,)\s*(?:عمارة|بناية|رقم)?\s*\d+',
                r'\b[A-Za-z\u0600-\u06FF\s]+-\s+[A-Za-z\u0600-\u06FF\s]+(?:St\.?|Street)\s*-\s*(?:Bldg|Building)\s+\d+\b', r'\b(?:[A-Z][a-z]+\s*){2,}-\s+[A-Z][a-z]+\s+(?:St\.?|Street)\b',
                r'[\u0600-\u06FF]{2,}\s*[–-]\s*[\u0600-\u06FF]{2,}\s*[–-]\s*[\u0600-\u06FF\s]+', r'(?:شارع|St\.|Street)\s+[\u0600-\u06FFa-zA-Z\s]+(?:،|,)\s*(?:بناية|عمارة|Bldg\.?|Building)\s*\d+(?:،|,)?\s*[\u0600-\u06FFa-zA-Z\s]*',
                ...
                try:
                    for match in re.finditer(pattern, text, re.IGNORECASE | re.UNICODE):
                        ...
                        if not self._is_excluded(full_match_text):
                            entities.append({'start': match.start(0), 'end': match.end(0), 'text': full_match_text, 'entity_type': entity_type, 'score': 1.0, 'model': 'regex'})
                except re.error as e:
                    logger.warning(f"Regex error for pattern {pattern}: {e}")
        return entities

    # ... (بقية الدوال تبقى كما هي بدون أي تغيير) ...
    # ... The rest of the functions (_is_excluded, _is_valid_entity, etc.) remain exactly the same ...

    def _is_excluded(self, text: str) -> bool:
        if not text or not text.strip(): return True
        clean_text = text.strip().lower()
        if len(clean_text) <= 1: return True
        if clean_text.isdigit() and len(clean_text) <= 3: return True
        tokens = re.split(r'[\s/-]', clean_text)
        for token in tokens:
            if token in self.exclusion_list: return True
    ...
    def _is_valid_entity(self, entity: Dict) -> bool:
        ...
        if entity_type not in ['ID', 'PHONE', 'AGE', 'DATE'] and text.isdigit(): return False
        if entity_type not in ['ID', 'HOSP'] and len(text) <= 4 and text.isupper() and text.isalpha(): return False
        return True
    ...
    def merge_entities(self, all_entities: List[Dict], original_text: str) -> List[Dict]:
        ...
        for entity in all_entities:
            ...
            if entity['model'] == 'regex': entity['priority'] += 100
            elif entity['model'] == 'my_custom_ner_expert': entity['priority'] += 75
            elif 'gliner' in entity['model']: entity['priority'] += 25
            else: entity['priority'] += 0
        ...
        while i < len(entities):
            ...
            while j < len(entities) and entities[j]['start'] < max_end:
                if entities[j]['end'] <= max_end: current_cluster.append(entities[j])
            ...
            merged_entity = {'start': min(e['start'] for e in current_cluster), 'end': max(e['end'] for e in current_cluster), 'entity_type': winner['entity_type'], 'score': winner['score'], 'model': winner['model']}
    ...
    def _post_process_and_validate(self, entities: List[Dict], original_text: str) -> List[Dict]:
        ...
        dosage_keywords = {'dose', 'tablet', 'bid', 'tid', 'qid', 'ml', 'mg', 'gm', 'mcg', 'جرعة', 'حبة', 'قرص', 'decrease', 'increase', 'give', 'given'}
        numeric_context_keywords = {'times', 'sib', 'sibling', 'abortion', 'gravida', 'para', 'g', 'p', 'a', 'cs', 'nvd', 'pregnancies', 'deliveries'}
        for entity in entities:
            text_lower = entity['text'].lower().strip()
            if entity['entity_type'] in ['PERSON', 'HCW'] and entity['start'] > signature_zone_start: logger.info(f"🖋️ SIGNATURE identified (as {entity['entity_type']}): '{entity['text']}'")
    ...
    def format_entities_for_display(self, entities: List[Dict]) -> str:
        if not entities: return "No entities detected"
    ...
    def create_detailed_analysis_report(self, original_text: str, result: str, entities: List[Dict]) -> str:
        ...
        report_lines = [
            f"--- ⚙️ Detailed Analysis Report (V10.3 - The Precision Engine) ---", "", "📄 Original Text:",
            original_text, "", "✅ De-identified Result:", normalized_result, ""
        ]
        if entities:
